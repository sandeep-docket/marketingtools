...
// Background Gradient - Using PNG image for accurate rendering
const BackgroundGradient = ({ rotation }: { rotation: number }) => (
  <img 
...
function BannerGenerator() {
  ...
  const [rotation, setRotation] = useState(321)
  const [bgPosition, setBgPosition] = useState(50) // Slider value 0-100, maps to 40-60% actual position
  const [isDownloading, setIsDownloading] = useState(false)
  const [showCustomSettings, setShowCustomSettings] = useState(false)

  // Randomize background rotation and position
  const handleRandomize = () => {
    const randomRotation = Math.floor(Math.random() * (360 - 300 + 1)) + 300 // 300-360
    const randomPosition = Math.floor(Math.random() * 101) // 0-100 for slider
    setRotation(randomRotation)
    setBgPosition(randomPosition)
  }

  // Map slider value (0-100) to actual position (40-60%)
  const getActualBgPosition = (sliderValue: number) => {
    return 40 + (sliderValue / 100) * 20 // Maps 0-100 to 40-60
  }
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      // Find the blog description element and add export mode (solid color fallback)
      const descriptionEl = bannerRef.current.querySelector('.blog-description') as HTMLElement
      if (descriptionEl) {
        descriptionEl.classList.add('export-mode')
      }
      ...
      if (descriptionEl) {
        descriptionEl.classList.remove('export-mode')
      }
      bannerRef.current.style.transform = originalTransform
      
      const fileName = blogTitle
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-|-$/g, '')
        .substring(0, 50) || 'blog-banner'
  ...
  }, [blogTitle, isDownloading])

  // Calculate background left position (maps 0-100 slider to 40-60% actual)
  const bgLeftPercent = getActualBgPosition(bgPosition)
      ...
      <div className="controls-panel">
        ...
        <div className="form-group">
          ...
          <input
            ...
            placeholder="e.g., Docket Multi Agent"
          />
        </div>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            placeholder="e.g., The specialist advantage for B2B GTM teams"
          />
        </div>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            placeholder="e.g., Deploy specialized expertise for every audience"
          />
        </div>
        ...
        <div className="background-controls">
          <div className="background-controls-header">
            ...
            <button 
              className={`btn-custom-settings ${showCustomSettings ? 'active' : ''}`}
          ...
          {showCustomSettings && (
            <div className="custom-settings-panel">
              <div className="form-group">
                <label htmlFor="rotation">Background Rotation</label>
              ...
              <div className="form-group">
                <label htmlFor="bgPosition">Background Position</label>
                <div className="slider-container">
                  <input
                    type="range"
                    id="bgPosition"
                  ...
                  />
                  <span className="slider-value">{bgPosition}%</span>
                </div>
              </div>
            </div>
          )}
        ...
        >
          ...
          {isDownloading ? 'Generating...' : 'Download Banner (PNG)'}
        </button>
        
        <p className="hint">Banner size: 1660 × 962 pixels</p>
      </div>
        ...
        <div className="preview-wrapper">
          <div className="banner-scale-container">
            <div className="banner" ref={bannerRef}>
              {/* Background Gradient - SVG Triangle Shape */}
              <div 
                className="background-vector"
                style={{ left: `${bgLeftPercent}%` }}
              >
              ...
              <div className="text-container">
                <p className="product-title">{productTitle || 'Product Title'}</p>
                <h1 className="blog-title">{blogTitle || 'Blog Title'}</h1>
                <p className="blog-description">{blogDescription || 'Blog Description'}</p>
              </div>
              
              {/* Docket Logo */}
              <div className="docket-logo">
                <DocketLogo />
              </div>
            </div>
          </div>
